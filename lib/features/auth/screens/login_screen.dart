                    ...
                    children: [
                      ...
                      Row(
                          ...
                          children: [
                            InkWell(
                              onTap: () =>
                                  authProvider.onChangeRememberMeStatus(),
                              child: Padding(
                      ...
                      InkWell(
                        onTap: () {
                          Navigator.of(context)
                              .pushNamed(RouteHelper.getCreateAccount());
                        },
                        child: Padding(
                            padding: const EdgeInsets.all(8.0),
                            child: Row(
    ...
    if (email.isEmpty) {
      ...
      authProvider.login(email, password).then((status) async {
        if (!status.isSuccess && status.message == 'verification') {
          Navigator.of(context).pushNamed(
              RouteHelper.getVerifyRoute('sign-up', email, session: null));
        } else if (status.isSuccess) {
          if (authProvider.isActiveRememberMe) {
            authProvider.saveUserNumberAndPassword(UserLogData(
              countryCode: countryCode,
              phoneNumber:
                  (splashProvider.configModel?.emailVerification ?? false)
                      ? null
                      : _emailController!.text,
              email: (splashProvider.configModel?.emailVerification ?? false)
                  ? _emailController!.text
                  : null,
              password: password,
            ));
