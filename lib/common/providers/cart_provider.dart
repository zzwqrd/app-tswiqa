...
class CartProvider extends ChangeNotifier {
  ...
  calculateDeliveryAmount(String? orderType, bool kmWiseCharge,
    ...
    return _deliveryAmount = deliveryAmount;
  }
  ...
  getDeliveryChange(String? orderType, bool kmWiseCharge,
  ...
  void setCartQuantity(bool isIncrement, int? index,
      {bool showMessage = false, BuildContext? context}) {
    if (isIncrement) {
      _cartList[index!].quantity = _cartList[index].quantity! + 1;
      _amount = _amount + _cartList[index].discountedPrice!;
      if (showMessage) {
        showCustomSnackBarHelper('quantity_increase_from_cart'.tr,
            isError: false);
      }
    } else {
      _cartList[index!].quantity = _cartList[index].quantity! - 1;
  ...
  int? isExistInCart(CartModel? cartModel) {
    for (int index = 0; index < _cartList.length; index++) {
      if (_cartList[index].id == cartModel?.id) {
        print(
            '----${_cartList[index].variation?.toJson()}----${(cartModel?.variation?.type)}');
